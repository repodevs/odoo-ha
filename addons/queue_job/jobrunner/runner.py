...
* Optionally adjust your configuration through environment variables:
  ...
  - ``ODOO_QUEUE_JOB_JOBRUNNER_DB_PORT=5432``, default ``db_port``
    or ``False`` if unset.
  - ``ODOO_QUEUE_JOB_JOBRUNNER_DB_USER=odoo``, default ``db_user``
    or ``False`` if unset.
  - ``ODOO_QUEUE_JOB_JOBRUNNER_DB_PASSWORD=odoo``, default ``db_password``
    or ``False`` if unset.
...
def _connection_info_for(db_name):
    ...
    _logger.info(f"connection to `{connection_info.get('host')}` with user `{connection_info.get('user')}`")
...
def _async_http_get(scheme, host, port, user, password, db_name, job_uuid):
    ...
    def urlopen():
        ...
        try:
            ...
            set_job_pending()
            _logger.error(f"failed to establish a new connection to {url}, sleeping 5 seconds before retry")
            time.sleep(5)
        except Exception:
