...
DB_USER_SESSION="${DB_USER}_session"
...
if [ -n "$DB_USER" -a -n "$DB_PASSWORD" -a -e "${_AUTH_FILE}" ] && ! grep -q "^\"$DB_USER\"" "${_AUTH_FILE}"; then
  if [ "$AUTH_TYPE" != "plain" ]; then
     ...
     pass_session="md5$(echo -n "$DB_PASSWORD$DB_USER_SESSION" | md5sum | cut -f 1 -d ' ')"
  else
     pass="$DB_PASSWORD"
     pass_session="$DB_PASSWORD"
  fi
  echo "\"$DB_USER\" \"$pass\"" >> ${PG_CONFIG_DIR}/userlist.txt
  echo "\"$DB_USER_SESSION\" \"$pass_session\"" >> ${PG_CONFIG_DIR}/userlist.txt
...
port=${DB_PORT:-5432} ${CLIENT_ENCODING:+client_encoding = ${CLIENT_ENCODING}\n}

[users]
${DB_USER:-postgres} = pool_mode=${POOL_MODE:-transaction}
${DB_USER_SESSION:-postgres_session} = pool_mode=session
